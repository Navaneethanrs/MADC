...
export function drawPhoneScreen(ctx, platform, isLight = false) {
  const w = 300
  const h = 610
  ctx.clearRect(0, 0, w, h)

  if (isLight) {
    // Elegant Mint Light Screen background
    const bg = ctx.createLinearGradient(0, 0, 0, h)
    bg.addColorStop(0, '#f2faf4')
    bg.addColorStop(1, '#e3f3e7')
    ctx.fillStyle = bg
    ctx.fillRect(0, 0, w, h)

    ctx.fillStyle = '#062d17'
    ...
    ctx.strokeStyle = 'rgba(0,168,67,0.1)'
    ...
    ctx.fill()
    roundRect(ctx, 20, cardY, 260, 160, 14)
    ctx.strokeStyle = 'rgba(0,168,67,0.3)'
    ctx.stroke()

    ;['#ff5f56', '#ffbd2e', '#00a843'].forEach((c, i) => {
      ctx.fillStyle = c
      ctx.beginPath()
    ...
    const pColor = platform.color === '#00ff66' || platform.color === '#39ff14' ? '#008837' : platform.color
    ...
    ctx.shadowColor = 'rgba(0,168,67,0.3)'
    ctx.shadowBlur = 12
    ctx.fillText(platform.label, 150, 160)
    ctx.shadowBlur = 0
    ctx.textAlign = 'left'
  } else {
    // Original Cyber Dark Screen
    const bg = ctx.createLinearGradient(0, 0, 0, h)
...
export function makeGlowTexture(isLight = false) {
  const c = document.createElement('canvas')
  c.width = c.height = 256
  const ctx = c.getContext('2d')
  const grad = ctx.createRadialGradient(128, 128, 0, 128, 128, 128)
  if (isLight) {
    grad.addColorStop(0, 'rgba(0,168,67,0.65)')
    grad.addColorStop(0.4, 'rgba(0,168,67,0.22)')
    grad.addColorStop(1, 'rgba(0,168,67,0)')
  } else {
